...
public class Algorithm {
    // 吴昌昊在这里写你的代码，实现make，split，还原私钥，detect 4个函数
    // detect混合在还原里了
    ...
    // 计算阈值
    public static int calcThresh(int[] alpha, int k, int n){
        int t = 0;
    ...
    // 插入列向量
    public static void insertM(int n, int h, ArrayList<ArrayList<Integer>> s, ArrayList<Integer> temp){
        if(n == 0 && h == 0){
    ...
    public static boolean inList(int[] list, int target, int index){
    ...
    public static splitMatrix makeS(int k, int n){
        ...
        Algorithm.splitMatrix s = al.new splitMatrix();
                ...
                for(int y = 0; y < origin.length; y++){
                    ...
                    if(origin[x][y] == 0){
                        ...
                        for(int t = 0; t < newcol; t++){
                            // 填充
                            splitImages[i][m*x+t/m][m*y+t%m] = s.S0.get(randList[i][t]).get(i).intValue();                            ;
                    ...
                    if(origin[x][y] == 1){
                        ...
                        for(int t = 0; t < newcol; t++){
                            // 填充
                            splitImages[i][m*x+t/m][m*y+t%m] = s.S1.get(randList[i][t]).get(i).intValue();
    ...
    public static void main(String[] args) {
        ...
        for(int j = 0; j < recover.length; j++){
            for(int k = 0; k < recover[j].length; k++){
                System.out.print(recover[j][k]);
                System.out.print('\t');
            }
            System.out.print("\n");
        }
